...
- `krynoxCaptcha()` middleware and a `krynoxWidget()` embed helper.
- Reads the token from the request body, falling back to the `x-krynox-captcha` header.
- Visitor IP comes from Express `req.ip`, so it honours your `trust proxy` setting
  instead of trusting a client-settable `X-Forwarded-For`.

[0.1.0]: https://github.com/krynox-security/plugin-express/releases/tag/v0.1.0
